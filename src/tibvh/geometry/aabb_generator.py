...
def compute_plane_aabb(
    ...
    for i in range(8):
        world_vertex = (
            rotation
            @ ti.Vector(
                [local_vertices[i, 0], local_vertices[i, 1], local_vertices[i, 2]]
            )
            + position
        )
...
@ti.func
def compute_sphere_aabb(
    position: ti.types.vector(3, ti.f32), size: ti.types.vector(3, ti.f32)
) -> tuple:
    """计算球体AABB"""
    radius = size[0]
...
def compute_capsule_aabb(
    ...
    for i in range(6):
        world_point = (
            rotation @ ti.Vector([key_points[i, 0], key_points[i, 1], key_points[i, 2]])
            + position
        )
...
def compute_box_aabb(
    ...
    for i in range(8):
        world_vertex = (
            rotation @ ti.Vector([vertices[i, 0], vertices[i, 1], vertices[i, 2]])
            + position
        )
        aabb_min = ti.min(aabb_min, world_vertex)
        aabb_max = ti.max(aabb_max, world_vertex)
...
@ti.func
def aabb_local2wolrd(
    aabb_center: ti.types.vector(3, ti.f32),
