...
class LBVH:
    ...
    def __init__(
        ...
        # 莫顿码相关字段
        self.morton_codes = ti.Vector.field(
            2, ti.u32, shape=self.max_aabbs
        )  # [morton_code, original_index]
    ...
    def print_timing_stats(self):
        ...
        for step, stat in time_stats.items():
            percentage = (
                (stat["total_time"] / total_time * 100) if total_time > 0 else 0
            )
            print(f"{step}:")
            ...
            print(
                f"  最小/最大时间: {stat['min_time'] * 1000:.2f}ms / {stat['max_time'] * 1000:.2f}ms"
            )
                ...
                while stack_depth > 0:
                    ...
                    if self._point_in_node(points[i_q], node_idx):
                        ...
                        if (
                            ...
                            and self.nodes[node_idx].right == -1
                        ):
                            ...
                            if (
                                ...
                                and stack_depth < self.max_stack_depth
                            ):
    ...
    @ti.func
    def collect_intersecting_elements(
        self,
    ...
    def validate_tree_structure(self):
        ...
        for i in range(internal_nodes):
            ...
            if left_child == -1 or right_child == -1:
                result["errors"].append(
                    f"内部节点 {i} 缺少子节点: left={left_child}, right={right_child}"
                )
            ...
            if element_id_data[i] != -1:
                result["warnings"].append(
                    f"内部节点 {i} 有非-1的element_id: {element_id_data[i]}"
                )
            ...
            if left_child != -1 and left_child < total_nodes:
                if parents_data[left_child] != i:
                    result["errors"].append(
                        f"节点 {i} 的左子节点 {left_child} 的父指针错误: {parents_data[left_child]} != {i}"
                    )

            if right_child != -1 and right_child < total_nodes:
                if parents_data[right_child] != i:
                    result["errors"].append(
                        f"节点 {i} 的右子节点 {right_child} 的父指针错误: {parents_data[right_child]} != {i}"
                    )
        ...
        for i in range(leaf_nodes):
            ...
            if left_child != -1 or right_child != -1:
                result["errors"].append(
                    f"叶子节点 {leaf_idx} 有子节点: left={left_child}, right={right_child}"
                )
                leaf_node_issues += 1
            ...
            if element_id < 0 or element_id >= self.n_aabbs:
                result["errors"].append(
                    f"叶子节点 {leaf_idx} 的element_id无效: {element_id}"
                )
                leaf_node_issues += 1
            else:
                if element_id in valid_element_ids:
                    result["errors"].append(
                        f"element_id {element_id} 被多个叶子节点引用"
                    )
            ...
            if parent_idx != -1:
                if parent_idx >= total_nodes:
                    ...
                    if left_child != i and right_child != i:
                        result["errors"].append(
                            f"节点 {i} 的父节点 {parent_idx} 未将其作为子节点"
                        )
                        parent_child_errors += 1
        ...
        if len(root_nodes) == 1:
            ...
            completion_rate = (
                completed_internal_nodes / internal_nodes if internal_nodes > 0 else 1.0
            )
    ...
    def print_validation_results(self, validation_result=None):
        ...
        print("\n节点状态:")
        print(
            f"  有效内部节点: {result.get('internal_nodes_valid', 0)}/{result['internal_nodes']}"
        )
        print(
            f"  有效叶子节点: {result.get('leaf_nodes_valid', 0)}/{result['leaf_nodes']}"
        )
